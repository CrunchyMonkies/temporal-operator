...
// fakeWorkflowService answers DeleteSchedule with whatever the test asked for, recording the
// schedules it was called for.
...
func (s *fakeWorkflowService) DeleteSchedule(_ context.Context, req *workflowservice.DeleteScheduleRequest, _ ...grpc.CallOption) (*workflowservice.DeleteScheduleResponse, error) {
	s.deleted = append(s.deleted, req.GetScheduleId())
	if s.err != nil {
		return nil, s.err
	}
...
// fakeTemporalClient stands in for a frontend connection. Only the calls the deletion paths make
...
var _ = Describe("Namespace deletion", func() {
	...
	// The annotation exists for a cluster that is gone for good, and "gone" can mean the target
	// cluster it ran on is unreachable too: nothing on the way to the frontend may stand in front
	// of it, the target resolution included.
	It("drops the finalizer without resolving the cluster's target when force deletion is requested", func() {
		cluster := &v1beta1.TemporalCluster{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "force-delete-unresolvable-cluster",
				Namespace: "default",
			},
			Spec: v1beta1.TemporalClusterSpec{
				NumHistoryShards: 1,
		...
		plain := createTestNamespaceFor(ctx, "plain-delete-unresolvable", cluster.GetName(), nil, deletionFinalizer)
		...
		Expect(err).To(HaveOccurred())
		held := getTestNamespace(ctx, plainKey)
		...
		By("checking the annotation releases a namespace on the same cluster")
		annotations := map[string]string{forceDeleteAnnotation: "true"}
		forced := createTestNamespaceFor(ctx, "force-delete-unresolvable", cluster.GetName(), annotations, deletionFinalizer)
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(Equal(reconcile.Result{}))
		Expect(getTestNamespace(ctx, forcedKey).Finalizers).To(ConsistOf(metav1.FinalizerDeleteDependents))
	})
